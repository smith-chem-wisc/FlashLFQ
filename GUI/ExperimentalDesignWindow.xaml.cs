...
namespace GUI
{
    ...
    public partial class ExperimentalDesignWindow : Window
    {
        ...
        public ExperimentalDesignWindow(ObservableCollection<SpectraFileForDataGrid> spectraFilesObservableCollection)
        {
            InitializeComponent();
            DgQuant.DataContext = spectraFilesQuantSets;

            foreach (var file in spectraFilesObservableCollection)
            {
                spectraFilesQuantSets.Add(new ExperimentalDesignForDataGrid(file.FilePath));
            }

            if (spectraFilesObservableCollection.Any())
            {
                outputPath = Directory.GetParent(spectraFilesObservableCollection.First().FilePath).FullName;
                outputPath = Path.Combine(outputPath, "ExperimentalDesign.tsv");
            }
        ...
        private void WriteExperDesignToTsv(string filePath)
        {
            using (StreamWriter output = new StreamWriter(filePath))
            {
                ...
                {
                    output.WriteLine(Path.GetFileNameWithoutExtension(spectraFile.FileName) +
